...
"""
Since we are using bernoulli rewards, we have each arm having a certain probability of giving 0 or 1
We need to choose that arm the most which has the highest probability

The information about best arm and actual probabilities is obviously not available to the agent
"""
...
class MultiArmedBandit:
    ...
    def pull(self, arm:int) -> int:
        ...
        reward = 1 if np.random.random() < self.arms[arm] else 0    # bernoulli rewards
        self.cumulative_regret_array.append(self.cumulative_regret_array[-1] + self.best_arm - reward)
        # we chose the best arm because that is the expectation value of best reward
        return reward
...
class Agent:
    def __init__(self, time_to_run, bandit : MultiArmedBandit):
        ...
        self.rewards = []   # an array to store the rewards
        self.bandit = bandit
    ...
    def plot_reward_vs_time_curve(self):
        ...
        timesteps = np.arange(1, len(self.rewards) + 1) # basically defining a timestep for each pull
